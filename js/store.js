/* Where the day's leads live.
   ...
   Three copies of every record, in order of how much we trust them:
...
CXA.store = (function () {
  ...
  /* Column order. The Supabase table, the CSV and the stored copy all use this,
     so the two can be diffed after the event without reformatting. */
  var COLUMNS = [
  ...
  /* Called once per machine. Chrome remembers the file across a full quit, and
     asks to allow editing once per launch. */
  function chooseCsv() {
    if (!csvSupported()) {
